...
module.exports = function(schema, options) {
  ...
  schema.statics.withAccess = function(subject, perms, parents, callback) {
    if(parents && !_.isArray(parents)) parents = [parents];

    var subjectKeys = subject.getAccessKeys();

    var validParents = _.filter(parents, function(parent) {
      return parent && _.isEmpty(_.difference(perms, parent.getAccess &&
                                              subject.getAccess(parent)));
    });

    var parentKeys = _.map(validParents, function(parent) {
      return options.key.call(parent);
    });

    // this is *really* messy
    var keys = _.uniq(_.flatten(subjectKeys.concat(parentKeys)));
